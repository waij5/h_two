...
class Ajax extends Backend
{
    ...
    public function uploadCustomerImg($customerId = '')
    {
        Config::set('default_return_type', 'json');
        $file  = $this->request->file('file');
        $label = trim(strip_tags(input('label', '')));
        $weigh = @intval(input('weigh', 0));
        if (empty($file)) {
            $this->error("未上传文件或超出服务器上传限制");
        }
        ...
        if (empty($suffix) || !in_array($suffix, ['gif', 'jpg', 'jpeg', 'bmp', 'png'])) {
            $this->error('请上传jpg,bmp,png,gif类型的图片');
        }

        //保存路径， /uploads/customer/年月/日/顾客卡号_24位随机码.文件类型
        $uploadDir = '/uploads/customer/' . date('Ym') . DS . date('d') . DS;
        $fileName  = $customerId . '_' . date('His') . Random::alnum(10) . '.' . $suffix;
        ...
        if ($splInfo) {
            ...
            $customerImg = model("CustomerImg");
            if ($customerImg->save($params)) {
                $this->success('上传成功', null, [
                    'url'   => $uploadDir . $splInfo->getSaveName(),
                    'label' => $customerImg->label,
                    'id'    => $customerImg->id,
                ]);
            } else {
                $this->error('保存失败');
    ...
    {
        // 判断控制器和方法判断是否有对应权限
        // 只有超级管理员能通过$this->auth->check()
        if (!$this->auth->check('ajax/wipecache')) {
