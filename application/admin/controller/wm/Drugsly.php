...
class Drugsly extends Backend
{
    ...
    public function index(){
        ...
        {	
			...
			$tDefaultWhere = ['m.mprimary_type'=> '4', 'm.mbelong_type'=> '1'];		//主类型4：领药；所属类型：药品
			$lDefaultWhere = ['mf.mprimary_type'=> '4', 'mf.mbelong_type'=> '1'];
        ...
        if($this->request->isPost()){
        	...
        	if($params){
        		...
        		if($searchMan_num){
					...
					$stocklogData['slexplain'] = '领药单'.$params['man_num'];
					$stocklogData['slremark'] = $params['mremark'];
					$stocklogData['sldepartment'] = $params['mdepart_id'];
					$stocklogData['sltype'] = '4';		//变动状态=主类型：4领药
					$stocklogData['smalltype'] = '4';	//次类型
					...
					foreach ($mpro_num as $k => $v) {
						...
						if($floatNum <1 || $intNum != $floatNum){
							$this->error('请输入正确的领药数量！');
						}
						if(intval($v)>$overStock[$k]['lstock']){
							$this->error('领药数量不能大于当前库存数！');
						}
    ...
    public function edits($ids = NULL){
    	$row = db('wm_manifest')->alias('m')->field('m.*,c.ctm_name')->join('yjy_customer c','m.member_id = c.ctm_id','LEFT')
    	...
    	$list = db('wm_manlist')->alias('ml')
    			->field('ml.*,lotnum.lot_id,lotnum.lstock,lotnum.lotnum,lotnum.lproducer,lotnum.laddr,lotnum.lapprov_num,lotnum.lregist_num,pro.pro_name,pro.pro_unit,pro.pro_id,pro.pro_spec,u.name as uname')
