...
class Drugscj extends Backend
{
    ...
    public function index(){
        ...
        {	
			...
			$tDefaultWhere = ['mprimary_type'=> '3', 'mbelong_type'=> '1'];		//主类型3：冲减；所属类型：药品
			$lDefaultWhere = ['mf.mprimary_type'=> '3', 'mf.mbelong_type'=> '1'];
            $total = $this->model
                    ->where($tDefaultWhere)
    ...
    public function add(){
    	if($this->model->select()){
			...
			$this->view->assign("man_num", 'ypcj1');
        }
        ...
        if($this->request->isPost()){
        	...
        	if($params){
        		...
        		if($searchMan_num){
					...
					$stocklogData['slexplain'] = '药品冲减单'.$params['man_num'];
    ...
    public function proSearch(){
        ...
        if($this->request->isAjax()){
            ...
            if(preg_match('/[\x{4e00}-\x{9fa5}]/u', $keywords)>0){// 搜索产品名称关键词
