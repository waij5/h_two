...
class Drugsjh extends Backend
{
    ...
    public function index(){
        ...
        {	
			...
			$tDefaultWhere = ['mprimary_type'=> '1', 'mbelong_type'=> '1'];		//主类型：进货；所属类型：药品
			$lDefaultWhere = ['mf.mprimary_type'=> '1', 'mf.mbelong_type'=> '1'];
            $total = $this->model
                    ->where($tDefaultWhere)
    ...
    public function add(){
    	if($this->model->select()){
			...
			$this->view->assign("man_num", 'ypjh1');
        }
        ...
        if($this->request->isPost()){
        	...
        	if($params){
        		...
        		if($searchMan_num){
					...
					$stocklogData['slexplain'] = '药品进货单'.$params['man_num'];
        ...
        if($this->request->isAjax()){
            ...
            if($keywords && $depot){
                $list = db('project')->alias('p')
                        ...
                        ->where($where)
                        ->order('pro_spell','asc')
                        ->column('p.pro_id,p.pro_name,p.pro_spell,u.name,p.pro_spec,p.pro_amount,p.pro_cost,p.addr,p.regist_num,p.approv_num,p.producer');
                // var_dump($list);
                /*foreach ($list as $key => $va) {	//json将NULL数据转为null，处理成空'';
                	// empty($va['approv_num'])?
                	if(empty($va['approv_num'])){
                		$list[$key]['approv_num'] ='';
                	}
                }*/
            }
