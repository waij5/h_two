...
class Drugs extends Backend
{
    ...
    public function _initialize()
    {
		...
		$deptmentLists = model('Deptment')->where('dept_type','deduct')->where('dept_status','1')->order('dept_id', 'asc')->select();
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            $total = $this->model->alias('pro')
					...
					->where('pro.pro_type','1')
                    // ->order($sort, $order)
                    ->count();

            $list = $this->model->alias('pro')
                    ->field('pro.*, u.name as u_name, pd.pdc_name, pdc.pdc_name as pro_cat, depot.name as depot_name,dept_name')
                    ...
                    ->join('yjy_deptment dept', 'pro.dept_id = dept.dept_id', 'LEFT')
                    ->where($map)->where($where)
					->where('pro.pro_type','1')
    ...
    public function downloadprocess()
    {
        ...
        return $this->commondownloadprocess('DrugsReport', 'All business drugs statistic');
    }
...
//    改价
    ...
    public function alterDate(){
        $alter_manid = $this->request->post('alter_manid');
        if($alter_manid){
            $lastMonthFinalDay = strtotime(date('Y-m-1 23:59:59').'-1 day');     //上个月最后一天
            $nowMonth = strtotime(date('Y-m-1'));                //本月第一天
            \think\Db::startTrans();                //开启db回滚;
            $res = ['error' => false, 'msg' => '1'];

            $data = db('wm_manifest')->field('man_id,man_num,mcreatetime,msecond_type')->where('man_id',$alter_manid)->find();
// return json($data);
            if($data['mcreatetime'] >= $nowMonth){
                // return json($nowMonth);
                $mfRes = db('wm_manifest')->where('man_id',$alter_manid)->update(['mcreatetime'=> $lastMonthFinalDay]);
                $slRes = db('wm_stocklog')->where(['smalltype' => $data['msecond_type'], 'sletc' => $data['man_num']])->update(['sltime'=> $lastMonthFinalDay]);
