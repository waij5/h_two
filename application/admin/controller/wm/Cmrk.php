...
class Cmrk extends Backend
{
    ...
    public function index(){
        ...
        {	
			...
			$tDefaultWhere = ['mprimary_type'=> '2', 'mbelong_type'=> '4'];		//主类型2：入库；所属类型4：耗材
			$lDefaultWhere = ['mf.mprimary_type'=> '2', 'mf.mbelong_type'=> '4'];
            $total = $this->model
                    ->where($tDefaultWhere)
    ...
    public function add(){
    	if($this->model->select()){
			...
			$this->view->assign("man_num", 'hcrk1');
        }
        ...
        if($this->request->isPost()){
        	...
        	if($params){
        		...
        		if($searchMan_num){
					...
					$stocklogData['slexplain'] = '耗材入库单'.$params['man_num'];
