...
class Goodsjh extends Backend
{
    ...
    public function index(){
        ...
        {	
			...
			$tDefaultWhere = ['mprimary_type'=> '1', 'mbelong_type'=> '2'];		//主类型1：进货；所属类型2：物品
			$lDefaultWhere = ['mf.mprimary_type'=> '1', 'mf.mbelong_type'=> '2'];
            $total = $this->model
                    ->where($tDefaultWhere)
    ...
    public function add(){
    	if($this->model->select()){
			...
			$this->view->assign("man_num", 'wpjh1');
        }
        ...
        if($this->request->isPost()){
        	...
        	if($params){
        		...
        		if($searchMan_num){
					...
					$stocklogData['slexplain'] = '物品进货单'.$params['man_num'];
