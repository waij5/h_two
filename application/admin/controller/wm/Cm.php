...
class Cm extends Backend
{
    ...
    public function _initialize()
    {
		...
		$depotLists = model('Depot')->where('type','4')->where('status','normal')->order('id', 'asc')->select();
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            // var_dump(md5(md5('123456').'7475'));die();
            ...
            $total = $this->model->alias('pro')
					...
					->where('pro.pro_type','4')        //4.耗材
                    // ->order($sort, $order)
                    ->count();

            $list = $this->model->alias('pro')
                    ->field('pro.*, u.name as u_name, pd.pdc_name, pdc.pdc_name as pro_cat2, depot.name as depot_name')
    ...
    public function add(){
        if($this->model->select()){
            ...
            $nums = $num['id']+1;
            $hc_num = 'hc'.$nums;
            $this->view->assign("hc_num", $hc_num);
        }else{
            $this->view->assign("hc_num", 'hc1');
        }
