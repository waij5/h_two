...
class Goodsrk extends Backend
{
    ...
    public function index(){
        ...
        {	
			...
			$tDefaultWhere = ['mprimary_type'=> '2', 'mbelong_type'=> '2'];		//主类型2：入库；所属类型2：物品
			$lDefaultWhere = ['mf.mprimary_type'=> '2', 'mf.mbelong_type'=> '2'];
            $total = $this->model
                    ->where($tDefaultWhere)
    ...
    public function add(){
    	if($this->model->select()){
			...
			$this->view->assign("man_num", 'wprk1');
        }
        ...
        if($this->request->isPost()){
        	...
        	if($params){
        		...
        		if($searchMan_num){
					...
					$stocklogData['slexplain'] = '物品入库单'.$params['man_num'];
