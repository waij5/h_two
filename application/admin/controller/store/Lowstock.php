 ...
 * @icon fa fa-circle-o
 */
class Lowstock extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            $list = $this->model->alias('lowlist')
					->field('lowlist.*, lowlist.stock_low - lowlist.stock as buynum, lowlist.stock_top - lowlist.stock as maxbuynum')
                    ->where($where)
					->where('stock < stock_low')
                    ->order($sort, $order)
                    ->limit($offset, $limit)
