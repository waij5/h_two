 ...
 * @icon fa fa-circle-o
 */
class Goodstock extends Backend
{
    ...
    {   
        ...
        {   
            ...
            $list = model('Product')->where('depot_id',$depot_id)->where('status','normal')->where($where)->order('name', 'asc')->order('id', 'asc')->order($sort, $order)->limit($offset, $limit)->select();
    ...
    public function add(){
		if($this->model->select()){
			...
			$this->view->assign("order_num", 'wpjh1');
        }
                ...
                try{
                    if(empty($onum_res)){
    					...
    					if ($result !== false){
                            ...
                            $drugsRes = model('Product')->saveAll($drugsData);//保存生产、到期日期到登记表
    						$dataresult = model('PurchaseFlow')->saveAll($data);
    						if ($dataresult !== false){
                                $exp = '物品进货单:';  //stock_add_log
