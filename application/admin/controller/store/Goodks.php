  ...
  //       }
    ...
    public function add(){
		if($this->model->select()){
			...
			$this->view->assign("order_num", 'll1');
        }
                    ...
                    if(empty($onum_res)){
                        ...
                        if ($res['error'] == false) {
                            ...
                            if($data){
                                ...
                                if ($dataresult !== false){
                                    $exp = '科室领料单:';  //stock_add_log
                                    model('Stock')->stock_lladd_log($drugRows,$params,$exp,$remark);//记录数据到产品库存变动明细表
                                    $this->success();
                                }else{
    ...
    public function edit($ids = NULL)
    {
        ...
        {	
            ...
            if($proLists){
                ...
                $lists[$k]['cost'] = $proLists[0]['cost'];
                $lists[$k]['sizes'] = $proLists[0]['sizes'];
                $lists[$k]['unit'] = $proLists[0]['unit'];
                $lists[$k]['price'] = $proLists[0]['price'];
