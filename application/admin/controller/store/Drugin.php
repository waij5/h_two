...
class Drugin extends Backend
{
    ...
    public function add(){
		if($this->model->select()){
			...
			$this->view->assign("order_num", 'yprk1');
        }
    ...
    public function del($ids = "")
    {
        if ($ids)
        {   
            $exp = '删除药品入库单:';      //保存到stock_log的说明信息
            $params = db('purchase_order')->field('rk_type')->where('id', $ids)->find();
            // var_dump($params['rk_type']);die();
            switch ($params['rk_type']) {
                case '0':
                    $type = '111';    //删除入库
                    break;
