...
class Drugcj extends Backend
{
    ...
    public function add(){
		if($this->model->select()){
			...
			$this->view->assign("order_num", 'ypcj1');
        }
		if ($this->request->isPost()){
            ...
            if(!$drugs_id)
                $this->error('请选择产品！');
            // var_dump($this->request->post("expirestime/a"));exit();
			$drugRows = array();
