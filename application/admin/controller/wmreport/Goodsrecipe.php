 ...
 * @icon fa fa-circle-o
 */
class Goodsrecipe extends Backend
{
    /**
     * Unit模型对象
     */
    ...
    {
        parent::_initialize();
        $depotLists = model('Depot')->where('status','normal')->where('type','2')->order('id', 'asc')->select();
    ...
    public function index(){
        if($this->request->isPost()){
            ...
            // foreach ($res as $ke => $val) {
                ...
                //array_multisort 按规定数组对某个数组进行排序
            // }
            foreach ($data as $k => $v) {
                $res[$v['pro_id']][] = $v;
                if($v['sltype']==6){
                    $totalss['flnum'] += $v['slnum'];
                    $totalss['flcost'] += $v['slallcost'];
                }else{
                    $totalss['clnum'] += $v['slnum'];
                    $totalss['clcost'] += $v['slallcost'];
                }
            }
            $counts = [];
            foreach ($res as $key => $val) {
                ...
                // var_dump($res);
            $this->view->assign('res',$res);$this->view->assign('totalss',$totalss);
    ...
    public function downloadprocess()
    {
        ...
        return $this->commondownloadprocess('goodsrecipe', 'Goodsrecipe name', $whereAddon);
    }
