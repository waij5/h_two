...
class Check extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $total = db('Product')->alias('pt')->where('pt.stock','>','0')->where($where)->count();
    ...
    private function generateRow(&$currentExcelSheet, $lineNo, &$rowData)
    {
        ...
        $currentExcelSheet->setCellValue('B' . $lineNo, $rowData['code']);
        $currentExcelSheet->setCellValue('C' . $lineNo, $rowData['name']);
        $currentExcelSheet->setCellValue('D' . $lineNo, $rowData['lotnum']);
        $currentExcelSheet->setCellValue('E' . $lineNo, $rowData['stock']);
        $currentExcelSheet->setCellValue('F' . $lineNo, $rowData["sizes"]);
        $currentExcelSheet->setCellValue('G' . $lineNo, $rowData['unit']);
