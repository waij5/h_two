...
class Checks extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $total = db('project')->alias('p')->where('p.pro_stock','>','0')->where($where)->count();
        $list = db('project')->alias('p')
                ->field('p.pro_id, p.pro_name, p.pro_spell, p.pro_code, p.pro_stock, p.pro_spec, p.pro_unit, p.depot_id,p.pro_cat2, p.pro_cat1, d.name as dname, u.name as uname')
        ...
        $namePre  = '产品盘点表' . str_pad((string) $recordId, 11, '0', STR_PAD_LEFT) . '_';
    ...
    private function generateHeader(&$currentExcelSheet, $title, &$cmdRecord)
    {
        ...
        $currentExcelSheet->mergeCells('D1:G1');
        ...
        // $currentExcelSheet->mergeCells('H1');
        $currentExcelSheet->setCellValue('H1', date('Y-m-d H:i'));
        ...
        $currentExcelSheet->setCellValue('B2', '拼音码');
        $currentExcelSheet->setCellValue('C2', '类别');
        $currentExcelSheet->setCellValue('D2', '产品名称');
        $currentExcelSheet->setCellValue('E2', '库存');
        $currentExcelSheet->setCellValue('F2', '规格');
        $currentExcelSheet->setCellValue('G2', '单位');
        $currentExcelSheet->setCellValue('H2', '所属仓库');

        $letter = array('A2','B2','C2','D2','E2','F2','G2','H2');
    ...
    private function generateRow(&$currentExcelSheet, $lineNo, &$rowData)
    {
        ...
        $currentExcelSheet->setCellValue('B' . $lineNo, $rowData['pro_spell']);
        $currentExcelSheet->setCellValue('C' . $lineNo, $rowData['pro_cat1'].'*'.$rowData['pro_cat2']);
        $currentExcelSheet->setCellValue('D' . $lineNo, $rowData['pro_name']);
        $currentExcelSheet->setCellValue('E' . $lineNo, $rowData['pro_stock']);
        $currentExcelSheet->setCellValue('F' . $lineNo, $rowData['pro_spec']);
        $currentExcelSheet->setCellValue('G' . $lineNo, $rowData["uname"]);
        $currentExcelSheet->setCellValue('H' . $lineNo, $rowData['dname']);
