 ...
 * @icon fa fa-circle-o
 */
class Fprosets extends Backend
{
    ...
    public function comselectpop()
    {
        if ($this->request->isAjax()) {
            ...
            $total                                       = $this->model->alias('fprosets')->where($where)->order($sort, $order)->count();
            $list                                        = $this->model->alias('fprosets')->join(model('deptment')->getTable() . ' dept', 'fprosets.dept_id = dept.dept_id', 'LEFT')->where($where)->order($sort, $order)->limit($offset, $limit)->field('fprosets.id, fprosets.dept_id, fprosets.name, fprosets.pic, fprosets.desc, dept.dept_name')->select();
