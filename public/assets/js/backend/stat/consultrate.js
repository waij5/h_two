define(['jquery', 'bootstrap', 'backend', 'table', 'form','bootstrap-select'], function($, undefined, Backend, Table, Form,bootstrapSelect) {
    var Controller = {
        index: function() {
            ...
            //省市县绑定(不是ajax提交的表单需要单独绑定)
            Form.events.cxselect($('#f-commonsearch'));
 
            var postUrl = 'stat/consultrate/index';
            ...
            $('#btn-export').on('click', function() {
                var url = '/stat/consultrate/downloadprocess' + '?op=' + encodeURI(JSON.stringify(op)) + '&filter=' + encodeURI(JSON.stringify(currentFilter));
