define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'jstree'], function ($, undefined, Backend, Table, Form, undefined) {
    ...
    var Controller = {
        index: function () {
            ...
            //添加合并按钮事件监听
            var toolbar = Table.defaults.toolbar;
            $(toolbar).on('click', '#btn-merge', function () {
                var ids = Table.api.selectedids($("#table"));
                var mergeUrl = $.fn.bootstrapTable.defaults.extend.merge_url;
                // Fast.api.open(mergeUrl) + (ids.length > 0 ? (mergeUrl.match(/(\?|&)+/) ? "&ids=" : "/ids/") + ids.join(",") : ''), __('merge'));
