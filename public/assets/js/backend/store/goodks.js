define(['jquery', 'bootstrap', 'backend', 'table', 'form','bootstrap-select'], function($, undefined, Backend, Table, Form,bootstrapSelect) {
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'store/goodks/index',
                    add_url: 'store/goodks/add',
                    edit_url: 'store/goodks/edit',
                    // del_url: 'store/goodks/del',
                    multi_url: 'store/goodks/multi',
                    table: 'depot_outks',
                }
            });
        ...
        add: function () {
            ...
            //     var depart = $(this).val();
            //     var url = '/store/goodks/getLeader';
            //     // console.log(depart);
            //     $.ajax({
            //         type: 'POST',
            //         url: url,
            ...
            $('#selectedGoods').on('input', '.sto_num', function() {
        ...
        function productSearch(keywords){
            ...
            if(keywords && depot){
                var url = '/store/goodks/proSearch';
                $.ajax({
                    type: 'POST',
                    url: url,
        ...
        function selectProduct(i){
            ...
            $('#c-depot_id').change(function(){
                ...
                /*3.24选择物品显示修改  所属仓库改变后清除记录列表所有产品的id的变量;所属仓库改变后搜索框中内容清空*/
                ps = Array();
                $('.keyword').val('');
                $('#word').html('');
            ...
            $('#selectedGoods').on('click', '[name="goodsRemoveBtn"]', function() {
                ...
                /*3.24选择物品显示修改  物品列表删除单个物品时清除该记录id*/
            ...
            $('#clear').on('click', function() {
                ...
                /*3.24选择物品显示修改  物品列表清空时清除记录列表所有产品的id的变量*/
                ps = Array();
        ...
        edit: function () {
            ...
            $('#isPrint').click(function(){
                ...
                if(confirm('是否开始打印?')){
                    ...
                    $('#departName').html('领料科室：'+departName);
                    $('.printA').css('display','');
