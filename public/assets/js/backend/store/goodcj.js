define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'bootstrap-datetimepicker'], function($, undefined, Backend, Table, Form, undefined) {
	...
	var Controller = {
		index: function() {
			...
			Table.api.init({
				extend: {
					index_url: 'store/goodcj/index',
					add_url: 'store/goodcj/add',
					edit_url: 'store/goodcj/edit',
					// del_url: 'store/goodcj/del',
					multi_url: 'store/goodcj/multi',
		...
		add: function() {
			...
			$('#selectedGoods').on('input', 'input', function() {
        ...
        function productSearch(keywords){
            ...
            if(keywords && depot){
                var url = '/store/goodcj/proSearch';
                $.ajax({
                    type: 'POST',
                    url: url,
