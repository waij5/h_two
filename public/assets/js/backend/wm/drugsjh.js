define(['jquery', 'bootstrap', 'backend', 'table', 'form','bootstrap-datetimepicker','bootstrap-select'], function ($, undefined, Backend, Table, Form,undefined,bootstrapSelect) {
    ...
    var Controller = {
        index: function () {
            ...
            // Table.api.init({
            //     extend: {
            //         index_url: 'wm/drugsjh/index',
            //         add_url: 'wm/drugsjh/add',
            //         edit_url: 'wm/drugsjh/edit',
            //         multi_url: 'wm/drugsjh/multi',
            ...
            //             // {field: 'mremark', title: __('mremark')},
        ...
        renderList:function(visitType){
            ...
            table.bootstrapTable({
                ...
                onLoadSuccess: function(data){
                    ...
                    if(/^\d+$/.test(autoClickId)){
                        ...
                        $('.detailIframe').attr('src', '/wm/drugsjh/edits/ids/' + autoClickId);
                    }else{
            ...
            $('#table tbody').on('click', 'tr', function() {
                ...
                if(/^\d+$/.test(man_id)){
                    $('.detailIframe').attr('src', '/wm/drugsjh/edits/ids/' + man_id);
        ...
        function productSearch(keywords){
            ...
            if(keywords && depot){
                ...
                $.ajax({
                    ...
                    success: function(msg){
                        $('#word').html('');
                        // $('#word').append('<li class="onloading" style="color:#18bc9c;"><td style="color:red;">拼音码</td><<>>名称<<>>规格<<>>单位</li>');
                        $('#word').append('<tr><th width="23%">拼音码</th><th width="40%">名称</th><th width="22%">规格</th><th width="15%">单位</th></tr>');
                        // console.log(msg);
                        dataObj = msg;
                        // var id = '';
                        $.each(dataObj, function(index,item){
                            // $('#word').append('<li style="padding-top:5px" onmouseover="$(this).css(\'color\',\'#18bc9c\')" onmouseout="$(this).css(\'color\',\'#555555\')" data-index="' + item.pro_id + '">' + item.pro_spell + '< >'+ item.pro_name + '< >'+ item.pro_spec +'< >'+ item.name +'</li>');$(this).css(\'background-color\',\'#18bc9c\')
...
//                  $(appendEle).find('.datetimepicker').parent().css('position', 'relative');   //日期插件
                    ...
                    /*var totalHtml = '<tr class="clearNav "><td colspan="2"></td></tr>';
                    $('#selectedDrugs').append(totalHtml);*/
                }
            };
            // console.log(ps);   
        }
                ...
                if(confirm('是否确认本进货单修改备注?')){
                    // $('.getNum').click(function(){
                        ...
                        $.ajax({
                                type: 'POST',
                                url: url,
                                data: {manid:manid,mremark:mremark},
                                dataType: 'json',
                                success: function(msg){
                                    // alert(msg);
                                    if(msg == '1'){
