define(['jquery', 'bootstrap', 'backend', 'table', 'form','bootstrap-datetimepicker','bootstrap-select'], function ($, undefined, Backend, Table, Form,undefined,bootstrapSelect) {
    ...
    var Controller = {
        index: function () {
            ...
            /*Table.api.init({
                extend: {
                    index_url: 'wm/drugsrk/index',
                    add_url: 'wm/drugsrk/add',
                    edit_url: 'wm/drugsrk/edit',
                    multi_url: 'wm/drugsrk/multi',
        ...
        renderList:function(visitType){
            ...
            table.bootstrapTable({
                ...
                onLoadSuccess: function(data){
                    ...
                    if(/^\d+$/.test(autoClickId)){
                        ...
                        $('.detailIframe').attr('src', '/wm/drugsrk/edits/ids/' + autoClickId);
                    }else{
            ...
            $('#table tbody').on('click', 'tr', function() {
                ...
                if(/^\d+$/.test(man_id)){
                    $('.detailIframe').attr('src', '/wm/drugsrk/edits/ids/' + man_id);
        ...
        function productSearch(keywords){
            ...
            if(keywords && depot){
                ...
                $.ajax({
                    ...
                    success: function(msg){
                        $('#word').html('');
                        // $('#word').append('<li class="onloading" style="color:#18bc9c">拼音码<<>>名称<<>>规格<<>>单位</li>');
                        $('#word').append('<tr><th width="23%">拼音码</th><th width="40%">名称</th><th width="22%">规格</th><th width="15%">单位</th></tr>');
                        // alert(msg);
                        dataObj = msg;
                        // var id = '';
                        $.each(dataObj, function(index,item){
                            // $('#word').append('<li style="padding-top:5px" onmouseover="$(this).css(\'color\',\'#18bc9c\')" onmouseout="$(this).css(\'color\',\'#555555\')" data-index="' + item.pro_id + '">' + item.pro_spell + '< >'+ item.pro_name + '< >'+ item.pro_spec +'< >'+ item.name +'</li>');
