define(['jquery', 'bootstrap', 'backend', 'table', 'form','bootstrap-datetimepicker','bootstrap-select'], function ($, undefined, Backend, Table, Form,undefined,bootstrapSelect) {
    ...
    var Controller = {
        index: function () {
            ...
            /*Table.api.init({
                extend: {
                    index_url: 'wm/goodscj/index',
                    add_url: 'wm/goodscj/add',
                    edit_url: 'wm/goodscj/edit',
                    multi_url: 'wm/goodscj/multi',
        ...
        renderList:function(visitType){
            ...
            table.bootstrapTable({
                ...
                onLoadSuccess: function(data){
                    ...
                    if(/^\d+$/.test(autoClickId)){
                        ...
                        $('.detailIframe').attr('src', '/wm/goodscj/edits/ids/' + autoClickId);
                    }else{
            ...
            $('#table tbody').on('click', 'tr', function() {
                ...
                if(/^\d+$/.test(man_id)){
                    $('.detailIframe').attr('src', '/wm/goodscj/edits/ids/' + man_id);
            ...
            $('#editRemark').click(function(){
                if(confirm('是否确认对本冲减单进行修改?')){
                    // $('.getNum').click(function(){
                        ...
                        var url = '/wm/goodsjh/editRemark';    //进货、入库、冲减、科室领料通用方法
                        // alert(mremark);
                        $.ajax({
                                type: 'POST',
                                url: url,
