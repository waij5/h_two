define(['jquery', 'bootstrap', 'backend', 'table', 'form','bootstrap-datetimepicker','bootstrap-select'], function ($, undefined, Backend, Table, Form,undefined,bootstrapSelect) {
    ...
    var Controller = {
        ...
        renderList:function(visitType){
            ...
            Table.api.init({
                extend: {
                    index_url: 'wm/drugscflist/index',
                    edit_url: 'wm/drugscflist/edit',
                    multi_url: 'wm/drugscflist/multi',
            ...
            if(visitType != ''){
                var rCols = [
                    [
                        ...
                        {field: 'iffy', title: __('发药状态'),formatter: function(value, row, index) {
                            if(value==0){
                                return '已发药';
                            }else{
                                return '<b style="color:#e73caa">待发药</b>';
                            }
                        }},
            ...
            table.bootstrapTable({
                ...
                onLoadSuccess: function(data){
                    ...
                    if(/^\d+$/.test(autoClickId) && !depot_id && !dept_id){
                        ...
                        $('.detailIframe').attr('src', '/wm/drugscflist/edit/ids/' + autoClickId);
                        // console.log(autoClickId);
                    }
                    else if(/^\d+$/.test(autoClickId) && depot_id && dept_id){
                        $('#table tbody tr').css('background-color','');
                        autoClickIds.parents('tr').css('background-color','#6ad4bf');
                        $('.detailIframe').attr('src', '/wm/drugscflist/edit/ids/' + autoClickId+'/depot_id/'+depot_id+'/dept_id/'+dept_id);
                    }
                    else if(/^\d+$/.test(autoClickId) && depot_id && !dept_id){
                        $('#table tbody tr').css('background-color','');
                        autoClickIds.parents('tr').css('background-color','#6ad4bf');
                        $('.detailIframe').attr('src', '/wm/drugscflist/edit/ids/' + autoClickId+'/depot_id/'+depot_id);
                    }
                    else if(/^\d+$/.test(autoClickId) && !depot_id && dept_id){
                        $('#table tbody tr').css('background-color','');
                        autoClickIds.parents('tr').css('background-color','#6ad4bf');
                        $('.detailIframe').attr('src', '/wm/drugscflist/edit/ids/' + autoClickId+'/dept_id/'+dept_id);
                    }
                    else if(!autoClickId){
            ...
            $('#isCusPrint').click(function(){
                ...
                if (cStatuss.length >0)
                {
                    ...
                    var all_print_url='wm/drugscflist/cf_print';

                    if(!depot_id && !dept_id){
                        top.window.Fast.api.open(all_print_url + (all_print_url.match(/(\?|&)+/) ? "&ids=" : "/ids/") + cStatuss+'/printType/'+'1', 'ALL处方打印');             
                    }
                    else if(depot_id && dept_id){
                        top.window.Fast.api.open(all_print_url + (all_print_url.match(/(\?|&)+/) ? "&ids=" : "/ids/") + cStatuss+'/printType/'+'1'+'/depot_id/'+depot_id+'/dept_id/'+dept_id, 'ALL处方打印');
                    }
                    else if(depot_id && !dept_id){
                        top.window.Fast.api.open(all_print_url + (all_print_url.match(/(\?|&)+/) ? "&ids=" : "/ids/") + cStatuss+'/printType/'+'1'+'/depot_id/'+depot_id, 'ALL处方打印');
                    }
                    else if(!depot_id && dept_id){
                        top.window.Fast.api.open(all_print_url + (all_print_url.match(/(\?|&)+/) ? "&ids=" : "/ids/") + cStatuss+'/printType/'+'1'+'/dept_id/'+dept_id, 'ALL处方打印');
                    }
                    // console.log(depot_id+'-*-*'+dept_id);
                }else{
                    Toastr.error('请选择打印项！');
            ...
            $('#table tbody').on('click', 'tr', function(){
                ...
                if(/^\d+$/.test(customer_id) && !depot_id && !dept_id){
                    $('.detailIframe').attr('src', '/wm/drugscflist/edit/ids/' + customer_id);
                
                }else if(/^\d+$/.test(customer_id) && depot_id && dept_id){
                    $('.detailIframe').attr('src', '/wm/drugscflist/edit/ids/' + customer_id+'/depot_id/'+depot_id+'/dept_id/'+dept_id);
                }
                else if(/^\d+$/.test(customer_id) && depot_id && !dept_id){
                    $('.detailIframe').attr('src', '/wm/drugscflist/edit/ids/' + customer_id+'/depot_id/'+depot_id);
                }
                else if(/^\d+$/.test(customer_id) && !depot_id && dept_id){
                    $('.detailIframe').attr('src', '/wm/drugscflist/edit/ids/' + customer_id+'/dept_id/'+dept_id);
                }
        ...
        edit: function () {
            ...
            $('#selectedDrugs tbody').on('dblclick', 'tr', function() {
                ...
                if(/^\d+$/.test(dr_id) && status=='待发药'){
                    var edit_url='wm/drugscf/edit_one';
                    parent.window.Fast.api.open(edit_url + (edit_url.match(/(\?|&)+/) ? "&ids=" : "/ids/") + dr_id+(edit_url.match(/(\?|&)+/) ? "&type=" : "/type/") + 2, '发药'+' id '+ dr_id,{
                        callback:function(e){
                            if(e == '1'){
                                Toastr.success('发药成功！');
                                setTimeout(function(){
                ...
                if(/^\d+$/.test(dr_id) && status=='已发药'){
                    var edit_url='wm/drugscf/edit_two';
                    parent.window.Fast.api.open(edit_url + (edit_url.match(/(\?|&)+/) ? "&ids=" : "/ids/") + dr_id+(edit_url.match(/(\?|&)+/) ? "&type=" : "/type/") + 2, '撤药'+' id '+ dr_id,{
                        callback:function(e){
                            if(e == '1'){
                                Toastr.success('撤销成功！');
                                setTimeout(function(){
            ...
            $('#isPrint').click(function(){
                ...
                if (cStatus.length >0)
                {
                    ...
                    // console.log(cStatus);
                    var cf_print_url='wm/drugscflist/cf_print';
                    top.window.Fast.api.open(cf_print_url + (cf_print_url.match(/(\?|&)+/) ? "&ids=" : "/ids/") + cStatus, '处方打印');
                    // console.log(pRow);
                    // allRow.prop("checked", true);
                }else{
                    Toastr.error('请选择打印项！')
