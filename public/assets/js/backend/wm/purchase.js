define(['jquery', 'bootstrap', 'backend', 'table', 'form','bootstrap-datetimepicker','bootstrap-select'], function ($, undefined, Backend, Table, Form,undefined,bootstrapSelect) {
    ...
    var Controller = {
        ...
        renderList:function(visitType){
            ...
            Table.api.init({
                extend: {
                    index_url: 'wm/purchase/index',
                    add_url: 'wm/purchase/add',
                    edit_url: 'wm/purchase/edits',
                    multi_url: 'wm/purchase/multi',
            ...
            if(visitType != ''){
                var rCols = [
                    [
                        {field: 'man_id', title: 'ID'},
                        {field: 'man_num', title: '采购单号'},
                        {field: 'muid', title: '操作人'},
                        {field: 'sup_name', title: '供应商'},
                        {field: 'dpt_name', title: '所属仓库'},
                        {field: 'mstatus', title: '状态',formatter: function(value, row, index) {
                            if(value==1){
                                return '<a  href="javascript:;" class="btn bg-blue btn-add ">待核收</a>';
                            }else if(value==2){
                                return '<a href="javascript:;" class="btn btn-danger btn-del">已核收</a>';
                            }else{
                                return '<a href="javascript:;" class="btn btn-primary">已报废</a>';
                            }
                        }},
                        {field: 'mcreatetime', title: '采购日期',formatter: Table.api.formatter.datetime},
                    ]
                ];
            }
            ...
            table.bootstrapTable({
                ...
                onLoadSuccess: function(data){
                    ...
                    if(/^\d+$/.test(autoClickId)){
                        ...
                        $('.detailIframe').attr('src', '/wm/purchase/edits/ids/' + autoClickId);
                    }else{
            ...
            $('#table tbody').on('click', 'tr', function() {
                ...
                if(/^\d+$/.test(man_id)){
                    $('.detailIframe').attr('src', '/wm/purchase/edits/ids/' + man_id);
...
//                  $(appendEle).find('.datetimepicker').parent().css('position', 'relative');   //日期插件
        ...
        edits: function () {
            ...
            $('.delPurList').click(function(){

                if(confirm('是否确认删除本条产品?')){
                    // $('.getNum').click(function(){
                        var plid = $(this).data('index');
                        var url = '/wm/purchase/delPurList';
                        // alert(mremark);
                        $.ajax({
                                type: 'POST',
                                url: url,
            ...
            $('#scrap').click(function(){
                if(confirm('是否确认作废本采购单?')){
                    // console.log($(this).data('scrap'));
                    var scrapId = $(this).data('scrap');
                    var url = '/wm/purchase/scrap';
                        // alert(mremark);
                        $.ajax({
                                type: 'POST',
                                url: url,
            ...
            $('#verified').click(function(){
                if(confirm('是否确认核收本采购单?')){
                    // console.log($(this).data('scrap'));
                    var verified = $(this).data('verified');
                    var url = '/wm/purchase/verified';
                        // alert(mremark);
                        $.ajax({
                                type: 'POST',
                                url: url,
