define(['jquery', 'bootstrap', 'backend', 'table', 'form','bootstrap-select'], function($, undefined, Backend, Table, Form,bootstrapSelect) {
    ...
    var Controller = {
        index: function () {
            ...
            /*Table.api.init({
                extend: {
                    index_url: 'wm/goodsll/index',
                    add_url: 'wm/goodsll/add',
                    edit_url: 'wm/goodsll/edit',
                    multi_url: 'wm/goodsll/multi',
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        {field: 'nickname', title: __('mout_id')},
                        {field: 'dept_name', title: __('mdepart_id')},
        ...
        renderList:function(visitType){
            ...
            Table.api.init({
                extend: {
                    index_url: 'wm/cmll/index',
                    add_url: 'wm/cmll/add',
                    edit_url: 'wm/cmll/edits',
                    multi_url: 'wm/cmll/multi',
            ...
            table.bootstrapTable({
                ...
                onLoadSuccess: function(data){
                    ...
                    if(/^\d+$/.test(autoClickId)){
                        ...
                        $('.detailIframe').attr('src', '/wm/cmll/edits/ids/' + autoClickId);
                    }else{
            ...
            $('#table tbody').on('click', 'tr', function() {
                ...
                if(/^\d+$/.test(man_id)){
                    $('.detailIframe').attr('src', '/wm/cmll/edits/ids/' + man_id);
        ...
        function productSearch(keywords){
            ...
            if(keywords && depot){
                ...
                $.ajax({
                    ...
                    success: function(msg){
                        $('#word').html('');
                        $('#word').append('<tr><th width="12%">耗材编号</th><th width="27%">名称</th><th width="12%">批号</th><th width="10%">进价</th><th width="10%">可用库存</th><th width="8%">单位</th><th width="11%">规格</th><th width="12%">到期日期</th></tr>');
                        // alert(msg);
                        dataObj = msg;
                        // var id = '';
        ...
        edits: function () {
            ...
            $('#editRemark').click(function(){
                if(confirm('是否确认对本领取单进行修改?')){
                    // $('.getNum').click(function(){
                        var manid = $('#c-man_id').val();
                        var mout_id = $('#c-mout_id').val();
                        var mdepart_id = $('#c-mdepart_id').val();
                        var msupplier_id = $('#c-msupplier_id').val();
                        var mremark = $('#c-mremark').val();
                        var member_id = $('#field_ctm_id').val();
                        var url = '/wm/cmll/editRemark';    //科室领料方法
                        // alert(mremark);
                        $.ajax({
                                type: 'POST',
                                url: url,
                                data: {manid:manid,mremark:mremark,msupplier_id:msupplier_id,mout_id:mout_id,mdepart_id:mdepart_id,member_id:member_id},
                                dataType: 'json',
                                success: function(msg){
                                    // console.log(msg);
                                    if(msg == '1'){
            ...
            $('#alterLlDate').click(function(){
                if(confirm('是否确认本领取单进行反日期操作?')){
                    // $('.getNum').click(function(){
            ...
            $('#selectedDrugs').on('click', '.backGoods', function() {
                var mpro_num = $(this).data('mpro_num');
                var ml_id = $(this).data('ml_id');

                var backNum = '<input class="backnum" type="text" value="" name="backnum"  size="4" placeholder="退领数量" /> '+'<button type="button" class="btn btn-xs btn-success confirmss"  data-ml_id='+ml_id+' data-mpro_num='+mpro_num+' >确&nbsp;定</button>';
                $(this).parents('tr').find('.backNum').html(backNum);
                

            });

            $('#selectedDrugs').on('click', '.confirmss', function() {
                var mpro_num = $(this).data('mpro_num');
                var ml_id = $(this).data('ml_id');

                var url = '/wm/cmll/backGoods';
                var backnum = $(this).parents('tr').find('.backnum').val();
                var reg = /^\+?[1-9][0-9]*$/;
                var issetNum = mpro_num-backnum;

                // console.log(lot_ids+'--'+manid+'--'+mcost+'--'+mprice+'--'+mpro_num+'--'+backnum);
                if(reg.test(backnum)==true &&  issetNum>=0){

                    $.ajax({
                        type: 'POST',
                        url: url,
                        data: {ml_id:ml_id, backnum:backnum},
                        success: function(msg){
                            // console.log(msg);
                            if(msg == '1'){
                                Toastr.success('退领成功！');
                ...
                }else if(reg.test(backnum)==false){
                    Toastr.error('请输入正确的退领数量！');
                }else if(issetNum<0){
                    Toastr.error('退领数量不得大于领取数量！');
                }
