    ...
    var Controller = {
        ...
        renderList:function(visitType){
            ...
            table.bootstrapTable({
                ...
                onLoadSuccess: function(data){
                    ...
                    if(/^\d+$/.test(autoClickId)){
                        ...
                        $('.detailIframe').attr('src', '/wm/goodsll/edits/ids/' + autoClickId);
                    }else{
            ...
            $('#table tbody').on('click', 'tr', function() {
                ...
                if(/^\d+$/.test(man_id)){
                    $('.detailIframe').attr('src', '/wm/goodsll/edits/ids/' + man_id);
        ...
        function productSearch(keywords){
            ...
            if(keywords && depot){
                ...
                $.ajax({
                    ...
                    success: function(msg){
                        $('#word').html('');
                        $('#word').append('<tr><th width="12%">物品编号</th><th width="27%">名称</th><th width="12%">批号</th><th width="10%">进价</th><th width="10%">可用库存</th><th width="8%">单位</th><th width="11%">规格</th><th width="12%">到期日期</th></tr>');
                        // alert(msg);
                        dataObj = msg;
                        // var id = '';
        ...
        edits: function () {
            ...
            $('#editRemark').click(function(){
                if(confirm('是否确认对本领料单进行修改?')){
                    // $('.getNum').click(function(){
                        ...
                        var url = '/wm/goodsll/editRemark';    //科室领料方法
                        // alert(mremark);
                        $.ajax({
                                type: 'POST',
                                url: url,
            ...
            $('#selectedDrugs').on('click', '.backGoods', function() {
                ...
                var backNum = '<input class="backnum" type="text" value="" name="backnum"  size="4" placeholder="退料数量" /> '+'<button type="button" class="btn btn-xs btn-success confirmss"  data-ml_id='+ml_id+' data-mpro_num='+mpro_num+' >确&nbsp;定</button>';
            ...
            $('#selectedDrugs').on('click', '.confirmss', function() {
                ...
                if(reg.test(backnum)==true &&  issetNum>=0){
                    ...
                    Toastr.error('请输入正确的退料数量！');
                }else if(issetNum<0){
                    Toastr.error('退料数量不得大于领料数量！');
                }
            ...
            $('#isPrint').click(function(){
                ...
                if(confirm('是否开始打印?')){
                    ...
                    $('#mdepartName').html('领料科室：'+mdepartName);
                    $('.printA').css('display','');
