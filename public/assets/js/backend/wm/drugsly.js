define(['jquery', 'bootstrap', 'backend', 'table', 'form','bootstrap-select'], function($, undefined, Backend, Table, Form,bootstrapSelect) {
    ...
    var Controller = {
        index: function () {
            ...
            /*Table.api.init({
                extend: {
                    index_url: 'wm/drugsly/index',
                    add_url: 'wm/drugsly/add',
                    edit_url: 'wm/drugsly/edit',
                    multi_url: 'wm/drugsly/multi',
        ...
        renderList:function(visitType){
            ...
            table.bootstrapTable({
                ...
                onLoadSuccess: function(data){
                    ...
                    if(/^\d+$/.test(autoClickId)){
                        ...
                        $('.detailIframe').attr('src', '/wm/drugsly/edits/ids/' + autoClickId);
                    }else{
            ...
            $('#table tbody').on('click', 'tr', function() {
                ...
                if(/^\d+$/.test(man_id)){
                    $('.detailIframe').attr('src', '/wm/drugsly/edits/ids/' + man_id);
        ...
        edits: function () {
            ...
            $('#alterLyDate').click(function(){
                if(confirm('是否确认本领药单进行反日期操作?')){
            ...
            $('#selectedDrugs').on('click', '.backDrugs', function() {
                var mpro_num = $(this).data('mpro_num');
                var ml_id = $(this).data('ml_id');

                var backNum = '<input class="backnum" type="text" value="" name="backnum"  size="4" placeholder="退药数量" /> '+'<button type="button" class="btn btn-xs btn-success confirmss"  data-ml_id='+ml_id+' data-mpro_num='+mpro_num+' >确&nbsp;定</button>';
            ...
            $('#selectedDrugs').on('click', '.confirmss', function() {
                ...
                if(reg.test(backnum)==true &&  issetNum>=0){
                    ...
                    Toastr.error('请输入正确的退药数量！');
                }else if(issetNum<0){
                    Toastr.error('退药数量不得大于领药数量！');
                }
