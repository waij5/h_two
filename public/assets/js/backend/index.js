define(['jquery', 'bootstrap', 'table', 'backend', 'addtabs', 'adminlte', 'form', 'cookie'], function($, undefined, Table, Backend, undefined, AdminLTE, Form, Cookie) {
    var Controller = {
        index: function() {
            //窗口大小改变,修正主窗体最小高度
            ...
            $('.shiftDeSim').on('click', function() {
            	$('.menuDetail').toggleClass('hidden');
            	$('.menuSimple').toggleClass('hidden');
            });

            //
            $('#switch-simple-menu').on('click', function() {
                $.cookie('use_simple_menu', $(this).prop('checked') ? 1 : 0, {expires: 3650, path: '/'});
                $('.menuDetail').toggleClass('hidden');
                $('.menuSimple').toggleClass('hidden');
            });
            ...
            if ($('#i-just-login').length && $('#i-just-login').val()) {
                // Fast.api.open('customer/rvinfo/todayrevisitnotices', __('Notices'));
                Fast.api.open('general/notice/index', __('Notices'));
            }
            $('#h-message').on('click', function() {
                // Fast.api.open('customer/rvinfo/todayrevisitnotices', __('Notices'));
                Fast.api.open('general/notice/index', __('Notices'));
            })
            $('#h-bookcustomer').on('click', function() {
                // Fast.api.open('customer/Customerosconsult/bookcustomer', __('Notices'));
                Fast.api.open('general/notice/index', __('Notices'));
            })
